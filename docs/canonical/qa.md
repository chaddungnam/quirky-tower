...
- 2.5D 무대 회귀 테스트: 고정 직교 사선 카메라, 픽셀 `Sprite3D`, `StaticBody3D` 바닥, `CharacterBody3D` 플레이어, 경로별 `Area3D` 수, 실제 콜라이더 겹침, 깊이 이동·크기 예고 PASS
...
- Godot MCP 720×1280 직접 런타임: 홈, 세로 3경로, 방송사고 2.5D 장애물 접근, 실제 마우스 타이밍 클릭, FAIL 결과 팝업, 같은 층 재도전 PASS
- 현재 검증 구간 런타임 오류: 0
