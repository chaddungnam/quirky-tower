...
func _run() -> void:
	...
	assert(camera.projection == Camera3D.PROJECTION_ORTHOGONAL, "the corridor uses a fixed ortho camera")
	assert(absf(camera.rotation.x) > 0.1 and absf(camera.rotation.y) > 0.1, "camera is oblique on two axes")
	...
	assert(sprite.texture_filter == BaseMaterial3D.TEXTURE_FILTER_NEAREST, "pixel art keeps nearest filtering")
