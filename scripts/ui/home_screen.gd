...
func _ready() -> void:
	...
	tagline.add_theme_color_override("font_color", Tokens.color(self, Tokens.CREAM))
	tagline.add_theme_color_override("font_outline_color", Tokens.color(self, Tokens.NAVY))
	...
	Tokens.style_button(get_node("Content/PlayButton"), Tokens.CORAL, Tokens.MAIN_TOUCH_HEIGHT)
	Tokens.style_button(get_node("Content/SettingsButton"), Tokens.TEAL)
...
func _draw() -> void:
	var cream := Tokens.color(self, Tokens.CREAM)
	var navy := Tokens.color(self, Tokens.NAVY)
	var gold := Tokens.color(self, Tokens.GOLD)
	...
	draw_style_box(Tokens.panel_style(self, Tokens.NAVY, 28), tower_rect)
	...
	draw_circle(Vector2(antenna_x, tower_rect.position.y - 82.0), 18.0 + sin(_pulse * 2.0) * 4.0, gold)
